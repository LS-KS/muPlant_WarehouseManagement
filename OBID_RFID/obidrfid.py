...
import subprocess
import platform
#libfetcp = ctypes.LoadLibrary('./libfetcp.dll')
libfetcp = ctypes.cdll.LoadLibrary('./OBID_RFID/fetcp.dll')
#libfeisc = ctypes.LoadLibrary('./libfeisc')
libfeisc = ctypes.cdll.LoadLibrary('./OBID_RFID/feisc.dll')
...
def ping(ip, timeout=2):
    ...
    '''
    try:
        subprocess.check_output("ping -{} 1 {} -w {}".format('n' if platform.system().lower()=="windows" else 'n', ip, timeout), shell=True)
